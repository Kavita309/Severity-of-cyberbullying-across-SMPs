...
def train(x_text, labels, model_type, embedding, oversampling):
    ...
    return classification_model(features, labels, model_type)
...
def classification_model(X, Y, model_type):
    ...
    return get_mean_scores(np.array(scores))
...
def get_mean_scores(scores):
    results = []
    for i in range(N_CLASS):
        precision = scores[:, i].mean()
        recall = scores[:,  N_CLASS+i].mean()
        f1 = scores[:, N_CLASS*2+i].mean()
        result = [precision, recall, f1]
        results.append(result)
    return results


def plot_graph(precision, recall, f1_score, dataset, embedding):
    x = [1, 2, 3, 4]
    # labels for lines
    ...
    plt.plot(x, precision, color='green',  label = "Precision",linestyle='dashed', linewidth = 3,
             marker='o', markerfacecolor='black', markersize=7)

    plt.plot(x, recall, color='red', label = "Recall",linestyle='dashed', linewidth = 3,
             marker='o', markerfacecolor='black', markersize=7)

    plt.plot(x, f1_score, color='yellow', label = "F1 Score", linestyle='dashed', linewidth = 3,
             marker='o', markerfacecolor='black', markersize=7)
...
def run_all_models(dataset, data_file, embedding, is_oversample):
    ...
    for i in range(4):
        results = train(x_text, labels, MODEL_TYPES[i], embedding, is_oversample)
...
run_all_models("Twitter", twitter_data_file, EMBEDDING[0], False)
run_all_models("Twitter", twitter_data_file, EMBEDDING[1], False)
run_all_models("Twitter with oversampling", twitter_data_file, EMBEDDING[0], True)
run_all_models("Twitter with oversampling", twitter_data_file, EMBEDDING[1], True)

run_all_models("Formspring", formspring_data_file, EMBEDDING[0], False)
run_all_models("formspring with oversampling", formspring_data_file, EMBEDDING[0], True)
